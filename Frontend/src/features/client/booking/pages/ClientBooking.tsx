...
const ClientBooking = () => {
    ...
    const [discount, setDiscount] = useState<{ id: string; percent: number; label: string } | null>(null);
    ...
    const fetchRoom = useCallback(async () => {
        ...
        try {
            ...
            const rtList: any[] = Array.isArray(roomTypesData) ? roomTypesData : [];
            const rpList: any[] = Array.isArray(roomPricesData) ? roomPricesData : [];
            const rtMap = new Map<string, any>(rtList.map((r) => [r.id, r]));
    ...
    const handleApplyDiscount = async () => {
        ...
        setDiscount(null);
        try {
            const list: any[] = await promotionApi.getPromotions();
            const found = list.find(
                (p: any) => p.code?.toLowerCase() === discountCode.trim().toLowerCase()
            );
            if (found) {
                setDiscount({
                    id: found.id,
                    percent: Number(found.discount_percent ?? found.value ?? 0),
                    label: found.code,
                });
            } else {
    ...
    if (booked) return (
        <div className="min-h-screen bg-gray-50 flex items-center justify-center px-4">
            <div className="bg-white rounded-3xl shadow-xl p-10 max-w-md w-full text-center flex flex-col items-center gap-5">
                ...
                <p className="text-gray-500 text-sm leading-relaxed">
                    Cảm ơn bạn đã đặt phòng tại <span className="font-semibold text-amber-600">Aurora Hotel</span>.<br />
                    Chúng tôi sẽ xác nhận đặt phòng trong thời gian sớm nhất.
                </p>
                <div className="w-full bg-gray-50 rounded-2xl p-4 text-left flex flex-col gap-2 text-sm">
                    <div className="flex justify-between">
                        <span className="text-gray-500">Phòng</span>
                        <span className="font-semibold">{roomType?.name} — {room.room_number}</span>
                    </div>
                    <div className="flex justify-between">
                        <span className="text-gray-500">Check-in</span>
                        <span className="font-semibold">{checkin_at}</span>
                    </div>
                    <div className="flex justify-between">
                        <span className="text-gray-500">Check-out</span>
                        <span className="font-semibold">{checkout_at}</span>
                    </div>
                    <div className="flex justify-between">
                    ...
                    <div className="flex justify-between border-t border-gray-200 pt-2 mt-1">
                        ...
                        <span className="font-bold text-amber-600">{formatVND(total)}</span>
                    </div>
                </div>
                <button
                    onClick={() => navigate('/rooms')}
                ...
                <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                    ...
                    <div className="lg:col-span-2">
                        ...
                        {step === 0 && (
                            <div className="bg-white rounded-2xl border border-gray-200 p-6 flex flex-col gap-6">
                                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                                    <FaRegCalendarAlt className="text-amber-500" /> Chọn loại & thời gian đặt phòng
                                </h2>

                                {/* ── Loại đặt phòng ── */}
                                <div className="flex flex-col gap-2">
                                    <label className="text-sm font-medium text-gray-700">Loại đặt phòng</label>
                                    <div className="grid grid-cols-2 gap-3">
                                        ...
                                        >
                                            ...
                                            <div className="text-center">
                                                ...
                                                <p className="text-xs mt-0.5 opacity-70">{pricePerNight > 0 ? formatVND(pricePerNight) + ' / đêm' : 'Liên hệ'}</p>
                                            </div>
                                            {booking_type === 'daily' && (
                                                <span className="text-xs bg-amber-500 text-white px-2 py-0.5 rounded-full">Đang chọn</span>
                                            )}
                                        </button>
                                        ...
                                        >
                                            ...
                                            <div className="text-center">
                                                ...
                                                <p className="text-xs mt-0.5 opacity-70">{pricePerHour > 0 ? formatVND(pricePerHour) + ' / giờ' : 'Liên hệ'}</p>
                                            </div>
                                            {booking_type === 'hourly' && (
                                                <span className="text-xs bg-amber-500 text-white px-2 py-0.5 rounded-full">Đang chọn</span>
                                            )}
                                ...
                                {booking_type === 'daily' ? (
                                    <div className="flex flex-col gap-4">
                                            ...
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <input
                                                    ...
                                                    onChange={(e) => {
                                                        ...
                                                        if (e.target.value >= checkout_at) setCheckoutAt('');
                                                    }}
                                                    className="px-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 bg-gray-50"
                                                />
                                            </div>
                                            <div className="flex flex-col gap-1.5">
                                        ...
                                        {nights > 0 && (
                                            <div className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 px-4 py-2.5 rounded-xl">
                                                <FaRegCalendarAlt />
                                                <span>Thời gian lưu trú: <strong>{nights} đêm</strong></span>
                                            </div>
                                        )}
                                    </div>
                                ) : (
                                    <div className="flex flex-col gap-4">
                                        ...
                                        {/* Số giờ */}
                                        <div className="flex flex-col gap-1.5">
                                            <label className="text-sm font-medium text-gray-700">Số giờ thuê</label>
                                            <div className="flex items-center gap-3">
                                                <button
                                                    type="button"
                                                    onClick={() => setDurationHours(h => Math.max(1, h - 1))}
                                                    className="w-9 h-9 rounded-full border border-gray-300 flex items-center justify-center text-lg hover:bg-gray-100 transition-colors font-bold"
                                                >−</button>
                                                ...
                                                <button
                                                    ...
                                                    className="w-9 h-9 rounded-full border border-gray-300 flex items-center justify-center text-lg hover:bg-gray-100 transition-colors font-bold"
                                                >+</button>
                                                <span className="text-sm text-gray-400">giờ (tối đa 24 giờ)</span>
                                            </div>
                                        </div>
                                        <div className="flex items-center gap-2 text-sm text-amber-700 bg-amber-50 border border-amber-200 px-4 py-2.5 rounded-xl">
                                            ...
                                            <span>
                                                Từ <strong>{startTime}</strong> — đến <strong>
                                                    {(() => { const d = new Date(`${bookingDate}T${startTime}`); d.setHours(d.getHours() + durationHours); return d.toTimeString().slice(0,5); })()} 
                                                </strong> ({durationHours} giờ)
                                            </span>
                                        </div>
                                    </div>
                                )}
                                ...
                                <div className="flex flex-col gap-1.5">
                                    ...
                                    <div className="flex items-center gap-3">
                                        <button
                                            ...
                                            className="w-9 h-9 rounded-full border border-gray-300 flex items-center justify-center text-lg hover:bg-gray-100 transition-colors font-bold"
                                        >−</button>
                                        <span className="w-8 text-center font-semibold text-gray-800">{num_guests}</span>
                                        <button
                                            type="button"
                                            onClick={() => setNumGuests(g => Math.min(roomType?.max_guests ?? 10, g + 1))}
                                            className="w-9 h-9 rounded-full border border-gray-300 flex items-center justify-center text-lg hover:bg-gray-100 transition-colors font-bold"
                                        >+</button>
                                        <span className="text-sm text-gray-400">/ tối đa {roomType?.max_guests} khách</span>
                                    </div>
                                </div>
                                ...
                                <div className="flex justify-end pt-2">
                                    <button
                                        disabled={!step0Valid}
                                        onClick={() => setStep(1)}
                                        className="bg-amber-500 hover:bg-amber-600 disabled:bg-gray-300 disabled:cursor-not-allowed text-white font-bold px-8 py-2.5 rounded-xl transition-colors"
                                    >
                                        Tiếp theo →
                                    </button>
                                </div>
                        ...
                        {step === 1 && (
                            <div className="bg-white rounded-2xl border border-gray-200 p-6 flex flex-col gap-6">
                                <h2 className="text-lg font-bold text-gray-900 flex items-center gap-2">
                                    <MdOutlinePersonOutline className="text-amber-500 text-xl" /> Thông tin khách hàng
                                </h2>
                                ...
                                {existingCustomer ? (
                                    <div className="flex flex-col gap-3 p-4 bg-amber-50 border border-amber-200 rounded-xl">
                                        ...
                                        <div className="grid grid-cols-2 gap-3 text-sm">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-semibold text-gray-800">{customerProfile?.phone}</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-500 text-xs">Email</p>
                                                <p className="font-semibold text-gray-800">{customerProfile?.email}</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-500 text-xs">CCCD/CMND</p>
                                                <p className="font-semibold text-gray-800">{customerProfile?.id_card_number}</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-500 text-xs">Quốc tịch</p>
                                                <p className="font-semibold text-gray-800">{customerProfile?.nationality}</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-500 text-xs">Ngày sinh</p>
                                                <p className="font-semibold text-gray-800">{customerProfile?.date_of_birth ? new Date(customerProfile.date_of_birth).toLocaleDateString() : 'N/A'}</p>
                                            </div>
                                        </div>
                                    </div>
                                ) : (
                                    <div className="flex flex-col gap-4">
                                        <div className="flex items-start gap-2 p-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-800">
                                            <span>⚠️</span>
                                            <span>
                                                Tài khoản của bạn chưa có hồ sơ khách hàng. Vui lòng nhập thông tin dưới đây — thông tin sẽ được lưu cho những lần đặt phòng tiếp theo.
                                            </span>
                                        </div>
                                        ...
                                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                            ...
                                            <div className="flex flex-col gap-1.5">
                                                <label className="text-sm font-medium text-gray-700">Số CCCD / Hộ chiếu *</label>
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => setGuestForm(f => ({ ...f, id_card_number: e.target.value }))}
                                            ...
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => setGuestForm(f => ({ ...f, date_of_birth: e.target.value }))}
                                                        className="w-full pl-10 pr-4 py-2.5 border border-gray-200 rounded-xl text-sm focus:outline-none focus:ring-2 focus:ring-amber-300 bg-gray-50"
                                                    />
                                                </div>
                                            </div>
                                            ...
                                            <div className="flex flex-col gap-1.5">
                                                ...
                                                <div className="relative">
                                                    ...
                                                    <input
                                                        ...
                                                        onChange={(e) => setGuestForm(f => ({ ...f, nationality: e.target.value }))}
                                ...
                                <div className="flex flex-col gap-2">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        <button
                                            ...
                                            className="px-5 py-2.5 bg-gray-900 hover:bg-gray-700 disabled:bg-gray-300 disabled:cursor-not-allowed text-white text-sm font-semibold rounded-xl transition-colors"
                                        >
                                    ...
                                    {discount && (
                                        <p className="text-green-600 text-xs flex items-center gap-1.5">
                                            <IoCheckmarkCircle /> Áp dụng thành công: Giảm {discount.percent}%
                                        </p>
                                    )}
                                ...
                                <div className="flex justify-between pt-2">
                                    <button
                                        onClick={() => setStep(0)}
                                        className="text-gray-500 hover:text-gray-700 text-sm font-medium px-4 py-2.5 rounded-xl border border-gray-200 hover:bg-gray-50 transition-colors"
                                    >
                                        ← Quay lại
                                    </button>
                                    <button
                        ...
                        {step === 2 && (
                            <div className="bg-white rounded-2xl border border-gray-200 p-6 flex flex-col gap-6">
                                <h2 className="text-lg font-bold text-gray-900">Xác nhận thông tin đặt phòng</h2>
                                    ...
                                    {booking_type === 'daily' ? (
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            <div>
                                                ...
                                                <p className="font-semibold text-gray-800">{checkin_at}</p>
                                                <p className="text-xs text-gray-400">Từ 14:00</p>
                                            </div>
                                            <div>
                                                <p className="text-gray-500">Trả phòng</p>
                                                <p className="font-semibold text-gray-800">{checkout_at}</p>
                                        ...
                                        <div className="grid grid-cols-2 gap-4 text-sm">
                                            ...
                                            <div>
                                                ...
                                                <p className="font-semibold text-gray-800">{startTime} — {(() => { const d = new Date(`${bookingDate}T${startTime}`); d.setHours(d.getHours() + durationHours); return d.toTimeString().slice(0,5); })()}</p>
                                                <p className="text-xs text-gray-400">{durationHours} giờ</p>
                                            </div>
                                        </div>
                                    )}
                                ...
                                <div className="flex flex-col gap-3 p-4 bg-gray-50 rounded-xl border border-gray-100">
                                    ...
                                    {isLoggedIn && authUser?.customers ? (
                                        <div className="grid grid-cols-2 gap-3 text-sm">
                                            <div><p className="text-gray-500">Họ tên</p><p className="font-semibold">{authUser.customers.full_name}</p></div>
                                            <div><p className="text-gray-500">Điện thoại</p><p className="font-semibold">{authUser.customers.phone}</p></div>
                                        </div>
                                    ) : (
                                        <div className="grid grid-cols-2 gap-3 text-sm">
                                            <div><p className="text-gray-500">Họ tên</p><p className="font-semibold">{guestForm.full_name}</p></div>
                                            <div><p className="text-gray-500">Điện thoại</p><p className="font-semibold">{guestForm.phone}</p></div>
                                            <div className="col-span-2"><p className="text-gray-500">Email</p><p className="font-semibold">{guestForm.email}</p></div>
                                            <div><p className="text-gray-500">CCCD</p><p className="font-semibold">{guestForm.id_card_number}</p></div>
                                            <div><p className="text-gray-500">Quốc tịch</p><p className="font-semibold">{guestForm.nationality}</p></div>
                                        </div>
                                    )}
                                    {notes && (
                                        <div><p className="text-gray-500 text-sm">Ghi chú</p><p className="font-semibold text-sm">{notes}</p></div>
                                    )}
                                </div>
                                ...
                                <div className="p-4 bg-blue-50 border border-blue-200 rounded-xl text-xs text-blue-700 leading-relaxed">
                                    ...
                                    <p>Miễn phí hủy trong vòng 24 giờ sau khi đặt. Hủy sau thời hạn sẽ bị tính phí 1 đêm đầu.</p>
                                </div>
                                ...
                                <div className="flex justify-between pt-2">
                                    ...
                                    <button
                                        ...
                                        className="bg-amber-500 hover:bg-amber-600 disabled:bg-amber-300 text-white font-bold px-8 py-2.5 rounded-xl transition-colors flex items-center gap-2"
                                    >
                                        {submitting && (
                        ...
                        <div className="bg-white rounded-2xl border border-gray-200 shadow-sm p-5 sticky top-24 flex flex-col gap-4">
                            ...
                            <div className="flex gap-3 items-start">
                                ...
                                <div>
                                    ...
                                    <p className="font-bold text-gray-900 text-sm mt-1">Phòng {room.room_number}</p>
                                    <div className="flex items-center gap-2 text-xs text-gray-500 mt-0.5">
                                        {/* <TbBed />
                                        <span>Tầng {room.floor}</span> */}
                                        {/* <span>·</span> */}
                                        <FaRegUser />
                                        <span>{roomType?.max_guests} khách</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="flex flex-col gap-2 text-sm">
                                {booking_type === 'daily' ? (
                                    ...
                                    <div className="flex justify-between text-gray-600">
                                        <span>{formatVND(pricePerHour)} × {durationHours} giờ</span>
                                ...
                                {discount && (
                                    <div className="flex justify-between text-green-600">
                                        <span className="flex items-center gap-1">
                                            <FaTag className="text-xs" /> Giảm {discount.percent}% ({discount.label})
                                        </span>
                            ...
                            <div className="flex justify-between items-center">
                                ...
                                <span className="font-bold text-xl text-amber-500">
                                    {nights > 0 ? formatVND(total) : durationHours > 0 ? formatVND(total) : '—'}
                                </span>
                            </div>

                            <p className="text-xs text-gray-400 text-center">Đã bao gồm thuế và phí dịch vụ</p>
                        </div>
                    </div>
                </div>
            </div>
