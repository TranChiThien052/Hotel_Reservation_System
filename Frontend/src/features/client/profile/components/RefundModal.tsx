    ...
    if (depositAmount > 0) {
        if (hoursElapsed <= 24) {
            return {
                ...
                isFullRefund: true,
                reason: 'Hủy trong vòng 24 giờ sau khi đặt — hoàn cọc 100%',
            };
        } else {
            return {
                amount: 0,
                isFullRefund: false,
                reason: 'Hủy sau 24 giờ — không được hoàn cọc',
            };
        }
    } else {
        if (hoursElapsed <= 24) {
            return {
                amount: totalAmount,
                isFullRefund: true,
                reason: 'Hủy trong vòng 24 giờ sau khi đặt — hoàn tiền 100%',
            };
        } else {
            const refund = Math.max(0, totalAmount - pricePerNight);
            return {
                amount: refund,
                isFullRefund: false,
                reason: `Hủy sau 24 giờ — trừ 1 đêm phí (${formatVND(pricePerNight)})`,
            };
        }
    }
};
