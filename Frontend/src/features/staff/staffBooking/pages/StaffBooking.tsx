...
const StaffBooking = () => {
    ...
    const bookingDetailPath = user?.role === 'manager' ? '/manager/bookings' : '/staff/bookings';
    ...
    const filterRoomStatus = (value: string) => {
        if (!value) { setFilteredBookings(bookingsData); return; }
        setFilteredBookings(bookingsData.filter((b) => b.status === value));
    };

    const handleSearch = (searchTerm: string) => {
        if (!searchTerm) { setFilteredBookings(bookingsData); return; }
        setFilteredBookings(bookingsData.filter((b) =>
    ...
    const columns: TableProps<Booking>["columns"] = [
        ...
        },
        {
            ...
            key: "checkin_at",
            render: (_, record) => <p>{record.checkin_at ? new Date(record.checkin_at).toLocaleDateString('vi-VN') : "-"}</p>,
            sorter: (a, b) => {
        ...
        },
        {
            ...
            key: "checkout_at",
            render: (_, record) => <p>{record.checkout_at ? new Date(record.checkout_at).toLocaleDateString('vi-VN') : "-"}</p>,
        },
        {
            title: "Trạng thái",
            dataIndex: "status",
            ...
            render: (text, record: Booking) => {
                const dynamicStatusItems: MenuProps["items"] = [
                    { key: "Pending", label: <span className="text-amber-600">Đang chờ</span>, onClick: () => handleStatusChange(record.id, { status: "pending" }) },
                    { key: "Confirmed", label: <span className="text-blue-600">Đã xác nhận</span>, onClick: () => handleStatusChange(record.id, { status: "confirmed" }) },
                    { key: "Completed", label: <span className="text-green-600">Hoàn thành</span>, onClick: () => handleStatusChange(record.id, { status: "completed" }) },
                    { key: "Cancelled", label: <span className="text-red-600">Đã hủy</span>, onClick: () => handleStatusChange(record.id, { status: "cancelled" }) },
                    { key: "Checked-in", label: <span className="text-cyan-600">Đã nhận phòng</span>, onClick: () => handleStatusChange(record.id, { status: "checked_in", actual_checkin_at: new Date().toISOString() }) },
                    { key: "Checked-out", label: <span className="text-purple-600">Đã trả phòng</span>, onClick: () => handleStatusChange(record.id, { status: "checked_out", actual_checkout_at: new Date().toISOString() }) },
                ];
                return (
                    <Dropdown menu={{ items: dynamicStatusItems }} trigger={["click"]} placement="bottomLeft">
                        <Tag
                            color={
                                text?.toLowerCase() === "pending" ? "amber"
                                : text?.toLowerCase() === "confirmed" ? "blue"
                                : text?.toLowerCase() === "completed" ? "green"
                                : text?.toLowerCase() === "cancelled" ? "red"
                                : text?.toLowerCase() === "checked_in" ? "cyan"
                                : text?.toLowerCase() === "checked_out" ? "purple"
                                : "default"
                            }
                            style={{ cursor: "pointer" }}
                        >
                            {text === "pending" ? "Đang chờ"
                                : text === "confirmed" ? "Đã xác nhận"
                                : text === "completed" ? "Hoàn thành"
                                : text === "cancelled" ? "Đã hủy"
                                : text === "checked_in" ? "Đã nhận phòng"
                                : text === "checked_out" ? "Đã trả phòng"
                                : "Đang dọn dẹp"}
                        </Tag>
                    </Dropdown>
                );
        ...
        },
        {
            ...
            render: (_, record) => (
                <Space size="small" wrap>
                    <Button onClick={() => navigate(`${bookingDetailPath}/${record.id}`)} type="primary" size="small">
                        Chi tiết
                    </Button>
                </Space>
            ),
        ...
        <div className="p-7 flex flex-col gap-5">
            <div className="flex items-center justify-between mt-3">
                <div className="flex flex-col gap-1">
                    ...
                    <p className="text-gray-600">Danh sách đặt phòng trong hệ thống khách sạn Aurora</p>
                </div>
                <div
            ...
            <div className="grid grid-cols-3 gap-5 w-2/3 mx-auto mt-4">
                <div className="bg-white rounded-lg border border-gray-300 shadow p-5 flex flex-col gap-3">
                    <div className="flex items-center gap-2 justify-between">
                        <span className="font-xl font-bold text-blue-500">Tổng đặt phòng</span>
                        <FaRegBuilding className="text-blue-500 text-2xl" />
                    </div>
                    <div className="text-2xl font-bold">{Array.isArray(bookingsData) ? bookingsData.length : 0}</div>
                </div>
                <div className="bg-white rounded-lg border border-gray-300 shadow p-5 flex flex-col gap-3">
                    <div className="flex items-center gap-2 justify-between">
                        <span className="font-xl font-bold text-green-500">Đã xác nhận</span>
                        <IoIosCheckmarkCircleOutline className="text-green-500 text-2xl" />
                    </div>
                    <div className="text-2xl font-bold">
                        {Array.isArray(bookingsData) ? bookingsData.filter((item: Booking) => item.status === "confirmed").length : 0}
                    </div>
                </div>
                <div className="bg-white rounded-lg border border-gray-300 shadow p-5 flex flex-col gap-3">
                    <div className="flex items-center gap-2 justify-between">
                    ...
                    <div className="text-2xl font-bold">
                        {Array.isArray(bookingsData) ? bookingsData.filter((item: Booking) => item.status === "pending").length : 0}
                    </div>
                </div>
            </div>
            ...
            <StaffBookingModal
                ...
                onSuccess={() => { fetchBookings(); message.success('Đặt phòng thành công!'); }}
