...
import { Button, Dropdown, message, Space, Tag, type MenuProps, type TableProps } from "antd";
...
const Services = () => {
    ...
    const handleStatusChange = async (id: string, updatedFields: Partial<ServiceFormData>) => {
        try {
            ...
            message.error("Có lỗi xảy ra khi cập nhật trạng thái dịch vụ. Vui lòng thử lại.");
        }
    };
    ...
    const handleSubmitForm = async (values: ServiceFormData) => {
        ...
        } else if (services.mode === FormModalModes.UPDATE && services.selectedRecord) {
          try {
    ...
    },
    {
      ...
      render: (_, record) => (
        <Space size="medium">
          <Button onClick={() => services.openEdit(record)}>
            Edit
          </Button>
        </Space>
      ),
    ...
    return (
        ...
        <div className="mt-5 border border-gray-300 rounded-lg">
          ...
          mode={services.mode}
          title={services.mode === FormModalModes.CREATE ? "Thêm dịch vụ mới" : "Chỉnh sửa dịch vụ"}
